...
# '''.format(self.output_path, self.output_path, nnodes, ntasks, ntasks_per_node, self.home_path, self.this_job_path, str(self.nprocs), self.user_id, argstr, self.job_id)
...
# '''.format(str(self.nprocs),self.job_path+'/train.py',argstr)
    ...
    def _request_submit_job(self):
        data = {
          ...
          'jobType': '81', # jobType 81 is for normal ai job(train,predict)
          'workspaceName': self.workspace_name,
          'tmpPath': self.workspace_real_path #self.workspace_path
        }
        if self.debug:
            print(data)        
